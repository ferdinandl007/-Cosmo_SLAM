...
xyaResampleVar = np.diag([10, 10, 10 * math.pi / 20000])
...
def runMCLLoop(robot: cozmo.robot.Robot):
	...
	while True:
		...
		# TODO not completely wrong, but not yet solving the problem
			newParticles = resampleIndependent(currentParticles, particleWeights, numParticles - 5 , xyaResampleNoise)
		...
		# 		to recover form mistakes (use sampleFromPrior from mcl_tools.py)
			ampleFromPrior = sampleFromPrior(mapPrior, 5)
		...
		# TODO Compare the independent re-sampling with "resampleLowVar" from mcl_tools.py
			...
			particl = resampleLowVar(particles,particleWeights,numParticles - 5,xyaResampleNoise)
